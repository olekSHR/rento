...
export default function Loading() {
  return (
    <main
      role="status"
      aria-live="polite"
      className={`min-h-screen bg-[#1B1B1B] ${BOTTOM_NAV_CONTENT_CLASS}`}
    >
        ...
        <div className="mx-auto flex h-[4rem] max-w-md items-center justify-between px-5">
          <div aria-hidden="true" className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0 space-y-2">
              ...
              <div className="h-2 w-14 animate-pulse rounded-md bg-white/5" />
            </div>
          </div>
          <div
            aria-hidden="true"
            className="h-11 w-16 shrink-0 animate-pulse rounded-full bg-white/10"
          />
        </div>
      </header>
      ...
      <div className="mx-auto max-w-md px-5 pb-6 pt-8">
        {Array.from({ length: 3 }).map((_, index) => (
          <div
            key={index}
            aria-hidden="true"
            className="mb-5 overflow-hidden rounded-[26px] border border-white/8 bg-[#2D2D2D]"
          >
            <div className="h-60 animate-pulse bg-white/5" />

            <div className="px-5 pb-5 pt-4">
              <div className="mb-3 h-5 w-4/5 animate-pulse rounded-lg bg-white/10" />
              ...
              <div className="flex items-end justify-between gap-4">
                ...
                <div className="h-10 w-24 animate-pulse rounded-xl bg-[#252525] ring-1 ring-white/10" />
              </div>
            </div>
          </div>
        ))}
